...
(async () => {
  ...
  await fsPromises.readFile(pathResolve(__dirname, '../List/domainset/reject_sukka.list'), { encoding: 'utf-8' }).then(data => {
    data.split('\n').forEach(line => {
      if (
        line.startsWith('#')
        || line.startsWith(' ')
  ...
  console.log(`Import ${sukkaSize} rules from reject_sukka.list!`);
  ...
  await fsPromises.readFile(pathResolve(__dirname, '../List/non_ip/reject.list'), { encoding: 'utf-8' }).then(data => {
    data.split('\n').forEach(line => {
  ...
  return fsPromises.writeFile(
    pathResolve(__dirname, '../List/domainset/reject.list'),
    `${[...domainSets].join('\n')}\n`,
    { encoding: 'utf-8' });
})();
