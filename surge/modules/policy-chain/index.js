...
let TITLE = `策略检测${NAMESPACE === 'xream' ? '' : `(${NAMESPACE})`}`

const NAME = `chains`

const KEY_DOMESTIC_LIMITED_URL = `@${NAMESPACE}.${NAME}.domestic_limited_url`
const KEY_DOMESTIC_URL = `@${NAMESPACE}.${NAME}.domestic_url`
const KEY_PROXY_URL = `@${NAMESPACE}.${NAME}.proxy_url`

const dataFile = `chains${NAMESPACE === 'xream' ? '' : `-${NAMESPACE}`}-box.dat`
const $ = new Env('chains', { dataFile })
...
const domesticLimitedUrl = $.getdata(KEY_DOMESTIC_LIMITED_URL) || `http://223.6.6.6`
const domesticUrl = $.getdata(KEY_DOMESTIC_URL) || `https://yiqing.hebi.gov.cn:8096/tool/hbcj10938.apk`
const proxyUrl = $.getdata(KEY_PROXY_URL) || `https://8.8.4.4`

let result = {}
!(async () => {
  const [{ chain: domesticLimitedChain = [] }, { chain: domesticChain = [] }, { chain: proxyChain = [] }] =
    await Promise.all([getDomesticLimitedChain(), getDomesticChain(), getProxyChain()])
  // $.log($.toStr(domesticLimitedChain))
  // $.log($.toStr(domesticChain))
  // $.log($.toStr(proxyChain))
  result = {
    title: `[国内受限] ${domesticLimitedChain.reverse().join(' ➟ ')}`,
    subt: `[国内] ${domesticChain.reverse().join(' ➟ ')}`,
    desc: `[国外] ${proxyChain.reverse().join(' ➟ ')}`,
  }
})()
  .catch(async e => {
    $.logErr(e)
  ...
  .finally(async () => {
    await notify(
      `${$.lodash_get(result, 'title')}`,
      `${$.lodash_get(result, 'subt')}`,
      `${$.lodash_get(result, 'desc')}`
    )
    $.done({
      ...arg,
      title: `[执行时间] ${new Date().toTimeString().split(' ')[0]}`,
      content: `${$.lodash_get(result, 'title')}\n${$.lodash_get(result, 'subt')}\n${$.lodash_get(result, 'desc')}\n`,
    })
  })

async function getDomesticLimitedChain() {
  let chain = []
  try {
    await $.http.get({
      url: domesticLimitedUrl,
      method: 'head',
    })
  } catch (e) {}
  try {
    ...
    const connection =
      connections.find(i => {
        ...
        return dest && domesticLimitedUrl.includes(dest)
      }) || {}
    chain = $.lodash_get(connection, 'metadata.chain') || []
  } catch (e) {}
  $.log(`domesticLimitedUrl: ${domesticLimitedUrl}`, chain.join(' ➟ '))
  return { chain }
}
async function getDomesticChain() {
  let chain = []
  try {
    await $.http.get({
      url: domesticUrl,
  ...
  try {
    ...
    const connection =
      connections.find(i => {
        const dest = $.lodash_get(i, 'metadata.host') || $.lodash_get(i, 'metadata.destinationIP')
        return dest && domesticUrl.includes(dest)
      }) || {}
    chain = $.lodash_get(connection, 'metadata.chain') || []
  } catch (e) {}
  $.log(`domesticUrl: ${domesticUrl}`, chain.join(' ➟ '))
  return { chain }
}
async function getProxyChain() {
  ...
  try {
    ...
    const connection =
      connections.find(i => {
        const dest = $.lodash_get(i, 'metadata.host') || $.lodash_get(i, 'metadata.destinationIP')
        return dest && proxyUrl.includes(dest)
      }) || {}
    chain = $.lodash_get(connection, 'metadata.chain') || []
  } catch (e) {}
  $.log(`proxyUrl: ${proxyUrl}`, chain.join(' ➟ '))
  return { chain }
}
