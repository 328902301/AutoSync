...
!(async () => {
  ...
  $.log('⏳ 本次 时间')
  $.log(timestamp)
  $.log(new Date(timestamp).toLocaleString('zh'))
  $.log('#️⃣ 本次 value')
  const value = $.lodash_get(body, 'unixtime')
  $.log(value)

  const last = $.getjson(KEY_LAST)
  // $.log('ℹ️ 上次数据')
  // $.log(last)
  // $.log($.toStr(last))
  if (!last) {
...
function formatDuration(number, precision) {
  if (number < 60) {
    return round(number, precision) + '秒'
  }
  if (number < 60 * 60) {
    return round(number / 60, precision) + '分钟'
  }
  if (number < 60 * 60 * 24) {
    return round(number / 60 / 60, precision) + '小时'
  }
  return round(number / 60 / 60 / 24, precision) + '天'
}
