...
(async () => {
  ...
  if (status == STATUS_COMING) {
    ////console.log(1)
    result["Disney"] =
      "<b>Disneyᐩ:</b> 即将登陆 ➟ " + "⟦" + flags.get(region.toUpperCase()) + "⟧ ⚠️";
    ...
    $task.fetch(option).then(
      (response) => {
        ...
        if (response.statusCode === 404) {
          ...
          result["Netflix"] =
            "<b>Netflix: </b>完整支持" + arrow + "⟦" + flags.get(region.toUpperCase()) + "⟧ 🎉";
          //$notify("nf:"+result["Netflix"])
