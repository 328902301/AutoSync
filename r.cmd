...
curl %curlFix%%curlproxy% --url https://raw.githubusercontent.com/DoingDog/DoingDog/main/cidr.exe -o cidr.exe
...
::deduplicate same DOMAIN-KEYWORD using uniq
echo ###Start deduplicate same DOMAIN-KEYWORD and DOMAIN-SUFFIX rule lines
busybox sed -i -E "/^$/d" bdfin.txt
busybox sed -i -E "/^ +$/d" bdfin.txt
busybox sed -n "/DOMAIN-SUFFIX,/p" bdfin.txt>>bdfin-do.txt
busybox sed -n "/DOMAIN-KEYWORD,/p" bdfin.txt>>bdfin-ds.txt
busybox sed -i "/DOMAIN-SUFFIX,/d" bdfin.txt
busybox sed -i "/DOMAIN-KEYWORD,/d" bdfin.txt
type bdfin-*.txt>>bdfn.txt
busybox sed -i -E "s/^[^,]+,//g" bdfn.txt
busybox sed -i -E "s/^[^,]+,//g" bdfin-do.txt
set LC_ALL='C'
busybox sort -i -o bdfbn.txt bdfn.txt
set LC_ALL=
busybox uniq -d bdfbn.txt>>bdfnn.txt
type bdfin-do.txt>>bdfnn.txt
set LC_ALL='C'
busybox sort -i -o bdfrn.txt bdfnn.txt
set LC_ALL=
busybox uniq -u bdfrn.txt>>bdfn2.txt
busybox sed -i -E "s/^/DOMAIN-SUFFIX,/g" bdfn2.txt
type bdfn2.txt>>bdfin.txt
type bdfin-ds.txt>>bdfin.txt
set LC_ALL='C'
busybox sort -u -i -o xn.txt bdfin.txt
set LC_ALL=
...
::generate Mosdns type rules
echo ###Start generate Mosdns type rules
copy /y bn.txt en.txt
busybox sed -i -E "/^[^D]/d" en.txt
busybox sed -i -E "/^D[^O]/d" en.txt
busybox sed -i -E "/KEYWORD/d" en.txt
busybox sed -i -E "s/^.*,//g" en.txt
for /f "tokens=2 delims=:" %%a in ('find /c /v "" en.txt')do set/a enrnum=%%a
echo # Mosdns total line %enrnum%>enr.txt
echo # Last updated %date% %time%>>enr.txt
type enr.txt
echo # -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*->>enr.txt
type en.txt>>enr.txt
copy /y enr.txt ..\fin-list.txt
...
busybox sed -i -E "/KEYWORD/d" fn.txt
busybox sed -i -E "s/^.*,/\|\|/g" fn.txt
busybox sed -i -E "s/$/\^/g" fn.txt
