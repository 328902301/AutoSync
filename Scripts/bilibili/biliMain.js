...
let blacklist = [];
if (read(blackKey)) {
  blacklist = read(blackKey).split(';');
} else {
  const defaultList = '';
  write(blackKey, defaultList);
  blacklist = defaultList.split(';');
}

const disableIndexStory = Boolean(read(bilibili_disable_index_story));
const enableMall = Boolean(read(bilibili_enable_mall));
    ...
    case /^https?:\/\/app\.bilibili\.com\/x\/v2\/splash\/list/.test($request.url):
      ...
      case /^https:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\/story\?/.test($request.url):
        try {
          ...
          write(storyAidKey, aid);
        } catch (err) {
          logError(`记录Story的aid出现异常：${err}`);
        }
        break;
      // 标签页处理，如去除会员购等等
      ...
      case /^https?:\/\/api\.bilibili\.com\/pgc\/page\/bangumi/.test($request.url):
        try {
          ...
          logError(`追番去广告出现异常：${err}`);
        }
        break;
      // 动态去广告
      case /^https?:\/\/api\.vc\.bilibili\.com\/dynamic_svr\/v1\/dynamic_svr\/dynamic_(history|new)\?/.test($request.url):
        try {
          let obj = JSON.parse($response.body);
          let cards = [];
      ...
      case /^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/skin\?/.test($request.url):
        ...
        try {
          ...
          logError(`热搜去广告出现异常：${err}`);
        }
        break;
        // 观影页去广告
      case /^https?:\/\/api\.bilibili\.com\/pgc\/page\/cinema\/tab\?/.test($request.url):
        try {
          let obj = JSON.parse($response.body);
          obj.result.modules.forEach((module) => {
            // 头部banner
