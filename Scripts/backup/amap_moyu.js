...
if (-1 != $request.url.indexOf("valueadded/alimama/splash_screen")) {
  ...
  obj.data &&
    ...
    obj.data &&
      ...
      obj.data.pull3.msgs &&
      (obj.data.pull3.msgs = []),
    obj.data && obj.data.mapBizList && (obj.data.mapBizList = []),
    $done({ body: JSON.stringify(obj) });
  ...
  obj.data && obj.data.header_hotword && (obj.data.header_hotword = []),
    $done({ body: JSON.stringify(obj) });
else if (-1 != $request.url.indexOf("ws/promotion-web/resource"))
  obj.data && obj.data.icon && (obj.data.icon = []),
    obj.data && obj.data.banner && (obj.data.banner = []),
    obj.data && obj.data.tips && (obj.data.tips = []),
    obj.data && obj.data.popup && (obj.data.popup = []),
    $done({ body: JSON.stringify(obj) });
else if (-1 != $request.url.indexOf("ws/msgbox/pull"))
  obj.msgs && (obj.msgs = []),
    obj.pull3 && obj.pull3.msgs && (obj.pull3.msgs = []),
    $done({ body: JSON.stringify(obj) });
else if (-1 != $request.url.indexOf("search/nearbyrec_smart")) {
  if (
    (obj.data && obj.data.coupon && delete obj.data.coupon,
    obj.data && obj.data.scene && delete obj.data.scene,
    obj.data && obj.data.activity && delete obj.data.activity,
    obj.data && obj.data.commodity_rec && delete obj.data.commodity_rec,
    obj.data && obj.data.modules)
  ) {
    let o = [];
