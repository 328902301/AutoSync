...
switch (isresponse) {
  ...
  case /^https?:\/\/m\.ithome\.com\/api\/news\/newslistpageget/.test(url):
    ...
    break;
  // IT之家 appRegex
  case /^https?:\/\/api\.ithome\.com\/json\/(listpage|newslist)\/news/.test(
    url
  ):
    try {
    ...
    break;
  // IT之家 appSlideRegex
  case /^https:\/\/api\.ithome\.com\/json\/slide\/index/.test($request.url):
    try {
    ...
    break;
  // IT之家 newAppFeed
  case /^https:\/\/napi\.ithome\.com\/api\/(news|topmenu)\/(getfeeds|index)/.test(
    $request.url
  ):
    try {
  ...
  // 微博 开屏广告 php
  case /^https?:\/\/(sdk|wb)app\.uve\.weibo\.com\/interface\/sdk\/sdkad.php/.test(
    url
  ):
    try {
      let tmp = /\{.*\}/;
    ...
    break;
  // 微博 开屏广告 lua
  case /^https?:\/\/(sdk|wb)app\.uve\.weibo\.com\/wbapplua\/wbpullad.lua/.test(
    url
  ):
    try {
      let obj = JSON.parse(body);
  ...
  case /^https?:\/\/hd\.mina\.mi\.com\/splashscreen\/alert/.test(url):
    ...
    break;
  // 小红书 开屏广告
  case /^https?:\/\/edith\.xiaohongshu\.com\/api\/sns\/v2\/system_service\/splash_config$/.test(
    url
  ):
    try {
      let obj = JSON.parse(body);
    ...
    break;
  // 小红书 处理信息流广告
  case /^https?:\/\/edith\.xiaohongshu\.com\/api\/sns\/v6\/homefeed\/categories\?/.test(
    url
  ):
    try {
      let obj = JSON.parse(body);
