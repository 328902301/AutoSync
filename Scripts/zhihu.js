...
// https://github.com/blackmatrix7/ios_rule_script/blob/master/script/zhihu/zhihu.js
...
(() => {
  ...
  if ($.isResponse) {
    switch (true) {
      // 知乎8.3.0移除推荐页顶部项
      case $.data.read("zhihu_settings_remove_sections", false) === true && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/feed-root\/sections\/query\/v2/.test($.request.url):
        response = removeFeedSections();
        break;
      // 回答内容优化
      case $.data.read("zhihu_settings_answer_tip", true) === true && /^https?:\/\/(www\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/appview\/v2\/answer\/.*(entry=(?!(preload-topstory|preload-search|preload-subscription)))?/.test($.request.url):
        response = modifyAnswer();
        break;
      // 处理登录用户信息
      case /^https:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/people\/self$/.test($.request.url):
        response = processUserInfo();
        break;
      // 黑名单增强 - 浏览黑名单用户信息时自动加入脚本黑名单
      case $.data.read("zhihu_settings_blocked_users", true) === true && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/people\/((?!self).)*$/.test($.request.url):
        response = autoInsertBlackList();
        break;
      // 推荐去广告与黑名单增强
      case /^https:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/topstory\/recommend\?/.test($.request.url):
        response = removeRecommendAds();
        break;
      // 关注列表去广告
      case /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/moments(\/|\?)?(recommend|action=|feed_type=)(?!\/people)/.test($.request.url):
        response = removeMomentsAds();
        break;
      // 回答列表去广告与黑名单增强
      case /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/(v4\/)?questions\/\d+/.test($.request.url):
        response = removeQuestionsAds();
        break;
      // 知乎V5版本评论去广告及黑名单增强
      case /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/comment_v5\/(answers|pins|comments?|articles)\/\d+\/(root|child)_comment/.test($.request.url):
        response = removeCommentV5Ads();
        break;
      // 知乎旧版回答中的评论黑名单增强
      case /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/(answers|pins|comments?|articles)\/\d+\/(root|child)_comments/.test($.request.url):
        response = removeCommentAds();
        break;
      // 知乎热榜去广告
      case $.data.read("zhihu_settings_hot_list", true) === true && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/topstory\/hot-lists?(\?|\/)/.test($.request.url):
        response = removeHotListAds();
        break;
      // 拦截官方账号推广消息
      case $.data.read("zhihu_settings_sys_msg", true) === true && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/notifications\/v3\/timeline\/entry\/system_message/.test($.request.url):
        response = removeSysMsgAds();
        break;
      // 屏蔽官方营销消息
      case $.data.read("zhihu_settings_sys_msg", true) != false && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/notifications\/v3\/message/.test($.request.url):
        response = removeMarketingMsg();
        break;
      // 去除预置关键字广告
      case $.data.read("zhihu_settings_preset_words", false) == true && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/search\/preset_words\?/.test($.request.url):
        response = removeKeywordAds();
        break;
      // 优化知乎软件配置
      case $.data.read("zhihu_settings_app_conf") == true && /^https?:\/\/appcloud2\.zhihu\.com\/v\d+\/config/.test($.request.url):
        response = modifyAppConfig();
        break;
      // 知乎热搜去广告
      case $.data.read("zhihu_settings_hot_search") == true && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/search\/top_search\/tabs\/hot\/items/.test($.request.url):
        response = removeHotSearchAds();
        break;
      // 黑名单管理
      case $.data.read("zhihu_settings_blocked_users") != false && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/settings\/blocked_users/.test($.request.url):
        manageBlackUser();
        break;
      default:
    }
    ...
    // 知乎屏蔽关键词解锁
    if ($.data.read("zhihu_settings_blocked_keywords") != false && /^https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/feed-root\/block/.test($.request.url) === true) {
      ...
      if ($.data.read("zhihu_settings_blocked_users") != false) {
        ...
        obj.data.forEach((comment) => {
          ...
          if (customBlockedUsers[commentUserName] || customBlockedUsers[replyUserName] ) {
            if (customBlockedUsers[commentUserName] && !replyUserName && $.request.url.indexOf("root_comment") > 0) {
              $.notification.debug(`屏蔽黑名单用户“${commentUserName}”的主评论。`);
            } else if (customBlockedUsers[commentUserName] && !replyUserName && $.request.url.indexOf("child_comment") > 0) {
              $.notification.debug(`屏蔽黑名单用户“${commentUserName}”的子评论。`);
            } else if (customBlockedUsers[commentUserName] && replyUserName && $.request.url.indexOf("child_comment") > 0) {
              $.notification.debug(`屏蔽黑名单用户“${commentUserName}”回复“${replyUserName}”的子评论。`);
            } else {
              $.notification.debug(`屏蔽“${commentUserName}”回复黑名单用户“${replyUserName}”的子评论。`);
            }
              ...
              comment.child_comments.forEach((childComment) => {
                if (customBlockedUsers[childComment.author.member.name] || customBlockedUsers[childComment.reply_to_author.member.name]) {
                  if (customBlockedUsers[childComment.author.member.name]) {
                    $.notification.debug(`屏蔽黑名单用户“${childComment.author.member.name}”的主评论。`);
                  } else {
                    $.notification.debug(`屏蔽“${childComment.author.member.name}”回复黑名单用户“${childComment.reply_to_author.member.name}”的子评论。`);
                  }
              ...
              comment.child_comments.forEach((childComment) => {
                ...
                if (customBlockedUsers[childCommentUserName] || blockCommentIdObj[childComment.reply_comment_id]) {
                  if (customBlockedUsers[childCommentUserName]) {
                    ...
                    $.notification.debug(`屏蔽“${childCommentUserName}”回复黑名单用户“${blockCommentIdObj[childComment.reply_comment_id]}”的子评论。`);
                  }
                  comment.child_comment_count -= 1;
                } else {
                  newChildComments.push(childComment);
...
function removeMomentsAds() {
  ...
  try {
    ...
    customBlockedUsers = !!customBlockedUsers ? customBlockedUsers : {};
    let data = [];
    const settings_moments_stream = $.data.read("zhihu_settings_moments_stream") == true;
    ...
    for (let i = 0; i < obj["data"].length; i++) {
      ...
      if (!element["ad"] && !element["adjson"] && !element["ad_list"]) {
        ...
        if (settings_blocked_users && element.target && element.target.origin_pin && element.target.origin_pin.author && customBlockedUsers[element.target.origin_pin.author.name]) {
          $.notification.debug(`屏蔽“${element.target.author.name}”转发黑名单用户“${element.target.origin_pin.author.name}”的想法。`);
        }
        // 屏蔽关注页的“最新视频”
...
function removeRecommendAds() {
  let response = null;
  try {
    const settings_remove_yanxuan = $.data.read("zhihu_settings_remove_yanxuan") == true;
    ...
    // 默认开启
    const settings_blocked_keywords = $.data.read("zhihu_settings_blocked_keywords") != false;
    const settings_blocked_users = $.data.read("zhihu_settings_blocked_users") != false;
    const user_info = getUserInfo();
    ...
    const dataFilter = (element) => {
      ...
      // 是否为广告
      let isAd = element["card_type"] === "slot_event_card" || element["card_type"] === "slot_video_event_card" || element.hasOwnProperty("ad") || (element["extra"] && element["extra"]["type"] === "Training");
      // 是否为流媒体
...
function manageBlackUser() {
  ...
  if ($.request.method == "GET") {
    try {
      ...
      if ($.request.url.indexOf("offset") < 0) {
        ...
        $.notification.post("脚本黑名单已清空，请滑动至黑名单末尾保证重新获取完成。");
      }
      let obj = JSON.parse($.response.body);
      if (!!obj["data"]) {
        ...
        if (obj["paging"]["is_end"] == true) {
          $.notification.post(`获取脚本黑名单结束，当前黑名单共${Object.keys(customBlockedUsers).length - defaultAnswerBlockedUsers.length}人。\n脚本内置黑名单${defaultAnswerBlockedUsers.length}人。`);
  ...
  // 移出黑名单
    ...
    try {
      let obj = JSON.parse($.response.body);
      if (obj.success) {
        let user_id = $.request.url.match(/https?:\/\/(api\.zhihu\.com|(103\.41\.167\.(226|234|235|236)))\/settings\/blocked_users\/([0-9a-zA-Z]*)/)[1];
        if (user_id) {
