...
console.log(`arg`, arg)
!(async () => {
  const res = await Promise.all([getIP(), getProxyIP()])
  console.log(`res`, res)

  const ip0 = $.lodash_get(res, '0.ip')
  const name0 = $.lodash_get(res, '0.name')
  const ip1 = $.lodash_get(res, '1.ip')
  const name1 = $.lodash_get(res, '1.name')
...
async function getProxyIP() {
  ...
  //     headers: { 'User-Agent': 'curl/7.16.3 (powerpc-apple-darwin9.0) libcurl/7.16.3' },
  //   })
  //   const statusCode = $.lodash_get(res, 'status')
  //   $.log('↓ res statusCode')
  //   $.log(statusCode)
  ...
  //   let name
  //    try {
  //     ip = body.match(/IP:(.*?)来自/)[1]
  //   } catch (e) {}
  //    try {
  //     name = body.match(/来自:(.*)/)[1]
  //   } catch (e) {}
  //   return { ip, name }
  // } catch (e) {
  //   $.log(e)
  ...
  try {
    ...
    $.log('↓ res body')
    $.log($.toStr(body))
    // 校验 status 或 body 内的数据
    const status = $.lodash_get(body, 'status')
    // if (!String(status).startsWith(2) || status !== 'success') {
    if (status !== 'success') {
      throw new Error(`${$.lodash_get(body, 'message') || '未知错误'}`)
    }
    return {
      ip: $.lodash_get(body, 'query'),
      name: [$.lodash_get(body, 'city'), $.lodash_get(body, 'isp').replace('Microsoft Corporation', '微软云')].join(
        ' '
      ),
    }
  } catch (e) {
